...
{
    public class BytehiveScraperContext : System.Xml.Xsl.XsltContext
    {
        private const string ExtensionsNamespaceUri = "http://xpathExtensions";
        private XsltArgumentList argList;

        public BytehiveScraperContext()
        {
        }

        public BytehiveScraperContext(NameTable nt, XsltArgumentList args)
            : base(nt)
        {
            argList = args;
        }
                                                         ...
                                                         string prefix, string name)
        {
            ...
            if (name.Equals("text") || name.Equals("charToCount") ||
                name.Equals("right") || name.Equals("left") || name.Equals("lowerChars") || name.Equals("upperChars"))
            {
